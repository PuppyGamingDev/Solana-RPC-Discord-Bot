...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("getminimumledgerslot")
        .setDescription("Fetch the lowest slot that the node has information about in its ledger.")
        .setDMPermission(false),
    async execute(interaction, client, connection) {
        await interaction.deferReply()
        try {
            const response = await connection.getMinimumLedgerSlot()
