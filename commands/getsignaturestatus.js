...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("getsignaturestatus")
        .setDescription("Fetch the current status of a signature")
        .addStringOption(option => option
            .setName('signature')
            .setDescription('Signature to query')
            .setRequired(true))
        .setDMPermission(false),
    async execute(interaction, client, connection) {
        await interaction.deferReply()
        try {
            const pubKey = interaction.options.getString('signature')
            const response = await connection.getSignatureStatus(pubKey)
