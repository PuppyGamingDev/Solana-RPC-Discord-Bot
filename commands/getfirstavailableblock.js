...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("getfirstavailableblock")
        .setDescription("Fetch the slot of the lowest confirmed block that has not been purged from the ledger")
        .setDMPermission(false),
    async execute(interaction, client, connection) {
        await interaction.deferReply()
        try {
            const response = await connection.getFirstAvailableBlock()
